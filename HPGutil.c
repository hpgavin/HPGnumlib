...
void clear_scr()                        /* clear the screen of all text */
{
#if ANSI_SYS
          printf("\033[2J"); /*  The escape character is 033 (33 octal) */
#endif
}
  

void scr_mode( int mode_code )          /* change the screen mode (?)   */
{
#if ANSI_SYS
        printf("\033[=%1dh",mode_code); 
#endif
        return;
}


void locate( int row, int column )      /* place the cursor at a coord. */
{
#if ANSI_SYS
        printf("\033[%1d;%1df",row,column);
#endif
        return;
}


void erase_line()                       /* erase a line from the cursor */
{
#if ANSI_SYS
          printf("\033[K");
#endif
...
/*
 * ESC_TO_EXIT  -  returns when [ESC] (ascii 27) is entered
 */
void enter_esc_to_exit(void)
{
  char ch = ';';
  color(0); color(1); color(32);
  fprintf(stderr,"  [ESC] [ENTER] to exit  ");
...
{
  FILE	*fp;
  char	pathToFile[MAXL], errMsg[2*MAXL];
...
/*
 * chOwnGrpMod - change the owner, group, and mode of a file
...
void chOwnGrpMod (const char *file_path, int mode)
{
  ...
  struct passwd *pwd;
  struct group  *grp;
  const char *usr_grp_name;

  pui = getpwuid(getuid());  
  if (pui == NULL) {
     printf("Failed to get pui \n");
  }
  usr_grp_name = pui->pw_name;
//printf("User name: %s\n", usr_grp_name);

  pwd = getpwnam(usr_grp_name);
  if (pwd == NULL) {
      printf("Failed to get uid \n");
  }
  uid = pwd->pw_uid;
  
  grp = getgrnam(usr_grp_name);
  if (grp == NULL) {
