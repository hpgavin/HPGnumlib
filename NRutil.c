/** @file
	...
	http://www.nr.com/public-domain.html
        updated - hpgavin - February 2022
*/

//nclude <unistd.h>    // symbolic constants and types, and related functions
//nclude <fcntl.h>     // file control
...
#include <math.h>
#include <time.h>
#include <unistd.h>    // symbolic constants and types, and related functions
#include <ctype.h>     // testing the type of variables 
...
/* allocate an int vector with subscript range v[nl..nh] */
{
	...
	v=(int32_t *)malloc((size_t) ((nh-nl+1+NR_END)*sizeof(uint32_t)));
...
/* allocate an int vector with subscript range v[nl..nh] */
{
	...
	v=(uint16_t *)malloc((size_t) ((nh-nl+1+NR_END)*sizeof(uint16_t)));
	if (!v) NRerror("allocation failure in u16vector()");
	return v-nl+NR_END;
}


uint32_t *u32vector(long nl, long nh)
/* allocate an int vector with subscript range v[nl..nh] */
{
	uint32_t *v;

	v=(uint32_t *)malloc((size_t) ((nh-nl+1+NR_END)*sizeof(uint32_t)));
	if (!v) NRerror("allocation failure in u32vector()");
	return v-nl+NR_END;
}
...
/* free an uint16_t vector allocated with ivector() */
{
	free((FREE_ARG) (v+nl-NR_END));
}

void free_u32vector(uint32_t *v, long nl, long nh)
/* free an uint32_t vector allocated with ivector() */
{
	free((FREE_ARG) (v+nl-NR_END));
}
...
/* allocate storage for a complex vector	*/
{
	...
	v=(f_complex *)malloc((unsigned) (nh-nl+1)*sizeof(f_complex));
	if (!v) NRerror("allocation failure in Cvector()");
	return v-nl;
}
...
/* allocate storage for a Complex matrix	*/
{
	...
	m=(f_complex **)malloc((unsigned) (nrh-nrl+1)*sizeof(f_complex*));
	if (!m) NRerror("allocation failure 1 in Cmatrix()");
	m -= nrl;
	for (i=nrl;i<=nrh;i++) {
		m[i]=(f_complex *)malloc((unsigned)(nch-ncl+1)*sizeof(f_complex));
...
void free_Cmatrix(f_complex **m, int nrl, int nrh, int ncl, int nch)
{
	...
	for(i=nrh;i>=nrl;i--) free((FREE_ARG) (m[i]+ncl));
	free((FREE_ARG) (m+nrl));
}
...
void free_D3matrix(float ***m, int nrl, int nrh, int ncl, int nch, int nzl, int nzh)
{
	...
	for ( r=nrh; r>=nrl; r-- ) {
		for ( c=nch; c>=ncl; c-- ) {
			free((FREE_ARG) (m[r][c]+nzl));
		}
                free((FREE_ARG) (m[r]+ncl) );
	}
        free((FREE_ARG) (m+nrl) );
}
