...
//void    free_vector();
  ...
  for(j=1;j<=m;j++) ui[j] = u[j][1];         // initial input
  (*ode_fctn)( dxdt1, t, x0, ui, yi, c );     // initial ode 
  for(i=1;i<=n;i++) x_sol[i][1] = x0[i];     // initial state
  for(i=1;i<=n;i++) x_drv[i][1] = dxdt1[i];  // initial dxdt
  for(k=1;k<=l;k++) y[k][1] = yi[k];         // initial output
  ...
  for ( p=1; p<N; p++ ) {
    ...
    for(j=1;j<=m;j++) ui[j] = u[j][p];
    for(i=1;i<=n;i++) xi[i] = x0[i] + dxdt1[i]*dt2;
    for(j=1;j<=m;j++) ui[j] = (u[j][p] + u[j][p+1])/2.0;
    ...
    for(j=1;j<=m;j++) ui[j] = u[j][p+1];

    (*ode_fctn)( dxdt4, time[p+1] , xi, ui, yi, c );

    for(i=1;i<=n;i++)
      x0[i] += (dxdt1[i]+2.0*(dxdt2[i]+dxdt3[i])+dxdt4[i])*dt/6.0; // update x0

    (*ode_fctn)( dxdt1, time[p+1], x0, ui, yi, c );

    for(i=1;i<=n;i++) x_sol[i][p+1] = x0[i];    // store state
    for(i=1;i<=n;i++) x_drv[i][p]   = dxdt1[i]; // store state derivitive
    for(k=1;k<=l;k++) y[k][p]       = yi[k];    // store output

    for(i=1;i<=n;i++) if(x_sol[i][p+1] > 1e30 || x_sol[i][p+1] < -1e30) break;
